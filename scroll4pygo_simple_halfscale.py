...
for fid in fragments:
    ...
    run_slug=f'training_scrolls_valid={fragment_id}_{CFG.size}x{CFG.size}_submissionlabels_wild1_scale2'
    ...
    trainer = pl.Trainer(
        ...
        #callbacks=[ModelCheckpoint(filename=f'scale2wild12_64_{fid}_{fold}_fr_{enc}'+'{epoch}',dirpath=CFG.model_dir,monitor='train/total_loss',mode='min',save_top_k=CFG.epochs),],
        callbacks=[ModelCheckpoint(filename=f'scale2wild12_64_{fid}_{fold}_fr_{enc}',dirpath=CFG.model_dir,monitor='train/total_loss',mode='min',save_top_k=1),], #CFG.epochs),],
    )
