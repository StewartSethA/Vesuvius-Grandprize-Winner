...
class CFG:
    ...
    rotate = A.Compose([A.Rotate(90,p=1)]) # PYGO: FULL 3D rotation!
...
for fid in fragments:
    ...
    if valid_mask_gt is None:
      valid_mask_gt = cv2.imread(CFG.comp_dataset_path + f"train_scrolls2/{fragment_id}/{fragment_id}_inklabels.png", 0)
    ...
    valid_ids = set(scroll4_ids)
    train_images, train_masks, train_xyxys, train_ids, valid_images, valid_masks, valid_xyxys, valid_ids = get_train_valid_dataset(CFG, train_ids, valid_ids, start_idx=CFG.start_idx, end_idx=CFG.end_idx, scale=CFG.scale)
    print(len(train_images))
    valid_xyxys = np.stack(valid_xyxys)
    train_dataset = CustomDataset(
        train_images, CFG, xyxys=train_xyxys, labels=train_masks, ids=train_ids, transform=get_transforms(data='train', cfg=CFG))
    valid_dataset = CustomDataset(
        valid_images, CFG,xyxys=valid_xyxys, ids=valid_ids, labels=valid_masks, transform=get_transforms(data='valid', cfg=CFG))
